...
const FetchPoolStatsHook = initialAddress => {
            ...
            try {
                ...
                fetchedData.forEach((pool: any) => {
                    ...
                    toNumberAttributes.forEach(attribute => {
                        poolItemNumbers[attribute] = parseFloat(poolItemNumbers[attribute]);
                    });

                    toNumberArrayAttributes.forEach(arrayAttribute => {
                        poolItemNumbers[arrayAttribute]?.forEach((stringValue, i) => {
                            poolItemNumbers[arrayAttribute][i] = parseFloat(
                                poolItemNumbers[arrayAttribute][i],
                            );
                        });
                    });

                    // push pool to custom object
                    poolGroups[poolId].push(poolItemNumbers);
                });
